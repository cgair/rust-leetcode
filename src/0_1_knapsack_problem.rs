...
// 
// 对于状态转移方程, 考虑到每个 dp[i][j] 都仅和 dp[i-1][j] 或 dp[i-1][j-wi]有关. 
// (第 i 个物品的状态往往只依赖于第 i-1 个物品的状态, 那么我们可以很轻松地想到只使用两行数组存储两个时刻的状态)
...
// 
// 与其把 dp[i - 1] 这一层拷贝到 dp[i]上, 不如只用一个一维数组: 只用dp[j]. 
...
// 
// 回想这个例子: N = 3, W = 4; wt = [2, 1, 3], val = [4, 2, 3].
// 
// 1. 使用二维 dp 数组时: 对于前 i 个物品, 当前背包的容量为 j, 这种情况下可以装的最大价值是 dp[i][j].
//                   [ 0  1  2  3  4    --> 容量 0～4
//                    [0, 0, 0, 0, 0],  --> 无物品
//                    [0, 0, 4, 4, 4],  --> 物品 0    由递推公式: dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - weight[i]] + value[i]);
//                    [0, 2, 4, 6, 6],  --> 物品 1    dp[i][j]由左上角方向推导而来 (包括正左和正上两个方向)
//                    [0, 2, 4, 6, 6]   --> 物品 2
//                   ]
// 2. 第 i 个物品的状态只依赖于第 i-1 个物品的状态 --> 只使用两行数组存储两个时刻的状态
//    此时 dp[i][j]: i 没有那么敏感了, 当前背包的容量为 j, 这种情况下可以装的最大价值是 dp[i][j].
//                   [[0, 0, 0, 0, 0], 
//                    [0, 0, 4, 4, 4]]  --> 物品 0
//                   [[0, 2, 4, 6, 6],  --> 物品 1
//                    [0, 0, 4, 4, 4]]
